...
(function () {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    if (falhas.length > 0) {
      ...
      const sugestoes = calcularSugestoes(falhas, valorMoto, entrada, rendaGlobal);
 ...
 */
   ...
   */
    const atualizarValoresParcelasDebounce = debounce(atualizarValoresParcelas, 2000); // Debounce
    function updateFinanceiro(source) {
      ...
      document.getElementById("btn-analise-final").classList.remove("v2-bg-black")
      document.getElementById("btn-analise-final").classList.add("v2-bg-black/50")
   ...
   */
    async function atualizarValoresParcelas(valorFinanciado) { // Ask gpt how to debounce
      ...
      if (income == 0) return;
      if (document.getElementById("v2-pagina-aprovado").classList.contains("v2-hidden")) return;
      const loanPayload = {
        credit: credit,
        income: income,
      }
      ...
      document.getElementById("btn-analise-final").classList.remove("v2-bg-black/50")
  ...
  window.addEventListener('DOMContentLoaded', () => {
    ...
    if (btnAnaliseFinal) {
      btnAnaliseFinal.addEventListener('click', async function() {
        const activatedInstallmentBtn = document.querySelector('[aria-pressed="true"]');
        if (!activatedInstallmentBtn) return 0;
        const chousenInstallmentAmount = parseInt(activatedInstallmentBtn.id.replace('btn-parcela-', ''), 10);
